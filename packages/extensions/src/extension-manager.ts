...
export interface Extension<TChat = unknown> {
    name: string;
    version?: string;
    description?: string;
    priority?: number;
    ...
    init?: (chat: TChat) => Promise<void> | void;
}

export class ExtensionManager<TChat = unknown> {
    private extensions: Extension<TChat>[] = [];

    register(extension: Extension<TChat>): void {
        this.extensions.push(extension);
        this.extensions.sort((a, b) => (b.priority || 0) - (a.priority || 0));
    }

    getExtensions(): Extension<TChat>[] {
        return [...this.extensions];
    }

    async init(chat: TChat): Promise<void> {
